...
from .models.sd_vae import StableDiffusionVAE
from .models.sd_unet import StableDiffusionUNet
from .models.sd_scheduler import StableDiffusionScheduler
from .models.sd_text_encoder import StableDiffusionTextEncoder
...
class StableDiffusionModel:
    def __init__(
        ...
        self.author = "GrafikXxxxxxxYyyyyyyyyyy"
        ...
        if model_path is None:
            if model_name is None:
                ...
                model_path = f"{self.author}/{model_type}_{model_name}"     

        # Инициализируем необходимые компоненты модели
        self.vae = StableDiffusionVAE(model_path, model_type, device)
        self.base = StableDiffusionUNet(model_path, model_type, device)
        self.text_encoder = StableDiffusionTextEncoder(model_path, model_type, device)
        self.scheduler = StableDiffusionScheduler(model_path, scheduler_name)
        if model_type == "sdxl": # Если SDXL то добавляется ещё и Refiner
            print(f"Loading refiner...")
            self.refiner = StableDiffusionUNet(
    ...
    def unet(self):
        return (
            self.refiner 
            if (self.type == "sdxl" and self.use_refiner) else
            self.base
        )
    ...
    def set_scheduler(self, scheduler_name):
        if self.scheduler_name != scheduler_name:
            self.scheduler = StableDiffusionScheduler(self.path, scheduler_name) 
            print(f"Scheduler has successfully changed to '{scheduler_name}'")



    # TODO: Используется во время обучения модели(!)
    def __call__(self):
        pass
