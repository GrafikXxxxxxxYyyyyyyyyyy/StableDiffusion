...
class DiffusionModel:
    """
    Данный класс представляет из себя обертку над всей диффузионной моделью
    Задумывается так, что хранит в себе и саму модель и планировщик шума
    Предоставляет функционал для forward and backward диффузионных процессов
    """
    def __init__(
        self,
        model_path: str,
        ...
        # А затем (если нужно), из его конфига инициализируется любой другой
    ...
    def denoising_value_valid(self, dnv):
    ...
    def retrieve_timesteps(
        ...
        # Дальше применяем denoising_ens/start если переданы, для регулировки числа шагов
        if denoising_start and self.denoising_value_valid(denoising_start):
        ...
        if (
            denoising_end
            and denoising_start
            and self.denoising_value_valid(denoising_end)
            and self.denoising_value_valid(denoising_start)
        ...
        elif denoising_end and self.denoising_value_valid(denoising_end):
            discrete_timestep_cutoff = int(
                round(
                    self.scheduler.config.num_train_timesteps
